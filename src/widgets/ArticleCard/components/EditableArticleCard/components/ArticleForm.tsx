...
interface ArticleFormProps {
  ...
  onCancel?: () => void; // Добавляем опциональный обработчик отмены
  autoSaveInterval?: number; // Интервал автосохранения в миллисекундах
  autoSaveKey?: string; // Ключ для сохранения в localStorage
}
...
const ArticleForm: React.FC<ArticleFormProps> = ({
  ...
  autoSaveKey = "article_form_draft",
}) => {
        ...
        <div className={styles.tagsList}>
          {fields.map((field, index) => (
            <div key={field.id} className={styles.tagItem}>
              ...
              {index === fields.length - 1 && renderAddButton()}
            </div>
          ))}
        </div>

        {fields.length === 0 && (
          <div className={styles.tagActions}>{renderAddButton()}</div>
        )}
      </div>
